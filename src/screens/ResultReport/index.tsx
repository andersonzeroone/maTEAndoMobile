...
import { ScrollView, View, TouchableOpacity, Platform, Text } from 'react-native';
...
export function ResultReport() {
  ...
  function getCompact(operation: string) {
    ...
    if (operation === 'Todos') {
      ...
      array = [mergeSubtracion,mergeAddtion,mergeDivision,mergeMultiplication]
      ...
      console.log('__________________________________________________________')
      console.log('1 - operação :', dataFilter.operation, 'taxa de acertos:', dataFilter.rateHit, '%')
  ...
  function mergerData(data: DataReport[], operation: string) {
    ...
    const arrayFilter = report.filter(data => (data.date >= dateInitial && data.date <= dateFinish && data.nameOperation === operation));
          ...
          <ContentFilter>
            ...
            <SelectContainer>
              <PickerSelect
                ...
                onValueChange={(itemValue) => setSelectedOperation(String(itemValue))}
              >
                ...
                <PickerSelect.Item label='Multiplicação' value='Multiplicação' />
              </PickerSelect>
            </SelectContainer>
          ...
          {filterReport.length > 0 ? (
            filterReport.map((item, index) => (
              <RowTable key={index} isDetach={(index % 2 == 0 ? true : false)}>
                <TextInfo>{dateInitial} - {dateFinish}</TextInfo>
